...
import { DocumentoEntregaType, TypoPagoEnum } from 'src/types/apps/transportType'
...
const getStats = (docsData: DocumentoEntregaType[]) => {
  ...
  const cashArr = docsData.filter(f => f.status === TransportStatusEnum.Entregado && f.tipoPago === TypoPagoEnum.Cash )
  const cash = cashArr.reduce((a,c) => a += c.bruto_E, 0)

  const checkArr = docsData.filter(f => f.status === TransportStatusEnum.Entregado && f.tipoPago === TypoPagoEnum.Check )
  const check = checkArr.reduce((a,c) => a += c.bruto_E, 0)
  console.log("checkArr", checkArr)

  const creditArr = docsData.filter(f => f.status === TransportStatusEnum.Entregado && f.tipoPago === TypoPagoEnum.Credit )
  const credit = creditArr.reduce((a,c) => a += c.bruto_E, 0)

  const transferArr = docsData.filter(f => f.status === TransportStatusEnum.Entregado && f.tipoPago === TypoPagoEnum.Transfer )
  const transfer = transferArr.reduce((a,c) => a += c.bruto_E, 0)

  const delivered = docsData.filter(f => f.status === TransportStatusEnum.Entregado)
  ...
  const donut = [cashArr.length, checkArr.length, creditArr.length, transferArr.length]
  return {
    total,
    cash,
    check,
    ...
    <Card>
      <CardHeader
        ...
        titleTypographyProps={{
          sx: { lineHeight: '1.5rem !important', letterSpacing: '0.15px !important' }
        }}
      />
      <CardContent>
        <Grid container sx={{ my: [0, 4, 3] }}>
          <Grid item xs={12} sm={6} sx={{ mb: [3, 0] }}>
            <ReactApexcharts type='donut' height={200} series={data.donut} options={options()} />
          </Grid>
          <Grid item xs={12} sm={6} sx={{ my: 'auto' }}>
            ...
            <Grid container>
              <Grid item xs={6} sx={{ mb: 4 }}>
                ...
                <Typography sx={{ fontWeight: 600 }}>{formatCurrency(data.cash)}</Typography>
              </Grid>
              <Grid item xs={6} sx={{ mb: 4 }}>
                <Box
                  sx={{
                ...
                <Typography sx={{ fontWeight: 600 }}>{formatCurrency(data.check)}</Typography>
              </Grid>
              <Grid item xs={6}>
                <Box
                  sx={{
                ...
                <Typography sx={{ fontWeight: 600 }}>{formatCurrency(data.transfer)}</Typography>
              </Grid>
              <Grid item xs={6}>
                <Box
                  sx={{
                ...
                <Typography sx={{ fontWeight: 600 }}>{formatCurrency(data.credit)}</Typography>
              </Grid>
            </Grid>
          </Grid>
        </Grid>
